...
/* Everything below is a question ABOUT an address. Without one the run would
   query the empty string against seven RPCs and a handful of HTTP APIs, and
   report their refusals as findings — forty lines of plausible-looking
   nothing. Say which input is missing instead, and say it first: job logs come
   back as a tail, but a run that stops here is short enough to read whole. */
if (!/^0x[0-9a-fA-F]{40}$/.test(TOKEN)) {
  console.error('');
  console.error('  No token address to ask about.');
  console.error('');
  console.error(`  TOKEN env var:          ${process.env.TOKEN ? process.env.TOKEN : '(unset)'}`);
  console.error(`  config.js contractAddress: ${CFG.contractAddress === null ? 'null' : JSON.stringify(CFG.contractAddress)}`);
  console.error('');
  console.error('  This repo ships with contractAddress null on purpose: it was copied');
  console.error('  from a sibling token, and a leftover address is a wrong answer that');
  console.error('  renders as a confident number rather than as a blank.');
  console.error('');
